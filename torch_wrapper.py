...
import numpy as np
from torch_utils import init_rnn, init_rnn_cell, init_fc,\
    gather_dim, unsort_dim
...
def time_distributed(inputs, op, activation=None):
    """
    apply op on both the batch (B) and time (T) dimension
    args:
        inputs: [B, T, P]
        op: a layer op that accepts [N, P]
    return:
        outputs: [B, T, P]
    """
    ...
    p_dim = inputs.size()[2:]
    outputs = op(inputs.view(-1, *p_dim))
